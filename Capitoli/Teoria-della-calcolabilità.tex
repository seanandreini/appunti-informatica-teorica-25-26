Hai perfettamente ragione, colpa mia! Un documento matematico impaginato in quel modo risulta pesante da leggere, e i simboli "incollati" alle lettere (come $\exists\mathcal{M}$) sono un pugno nell'occhio.Per risolvere il problema, ho fatto due cose:Ho inserito un comando \vspace{0.4cm} prima di ogni blocco importante (Definizione, Proposizione, Osservazione) per dare "respiro" al testo ed evitare l'effetto muro.Ho aggiunto uno spazio esplicito (usando \  in LaTeX) dopo i quantificatori, in modo che ora tu abbia cose pulite come $\exists\ \mathcal{M}$ e $\forall\ w$.Ecco la versione corretta e impaginata molto meglio:Snippet di codice\section{Teoria della Calcolabilità}
